...
def checkDoublets(adata, embedding='X_umap', cluster_key='leiden', doubletMethods=['scDblFinder','DoubletFinder','doubletdetection','scrublet']):
    f = plt.figure(figsize=(20,7),layout="constrained")
    sf = f.subfigures(1,3, width_ratios=(1,2,1.5))

    axs = sf[0].subplots(2,1)
    sc.pl.embedding(adata, basis=embedding, color = 'n_genes', vmax=7000, ax=axs[0], show=False)
    axs[1].remove()

    axs = sf[1].subplots(2,2)
    for n,method in enumerate(doubletMethods):
        sc.pl.embedding(adata, basis=embedding, color = f"predicted_doublet-{method}", ax=axs[n//2,n%2], show=False)

    ax = sf[2].subplots(1,1)
    doublet_df = pd.DataFrame([adata.obs.groupby(cluster_key)[f'predicted_doublet-{method}'].mean().sort_values() for method in doubletMethods]).T
    doublet_df.columns = doublet_df.columns.str.replace('predicted_doublet-','', regex=True)
    sns.heatmap(doublet_df, ax=ax,vmax=1)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=0)


def cluster_violinplot(
    adata: AnnData, markers: Iterable[str], group: str, f, layer: str = "normalized", useStripPlot=True
):
...
def cluster_stackedbarplot(
    ...
    if pct is False:
        ...
        ax.set_title(
            "Cluster Composition by Group (Counts)", fontsize=14, fontweight="bold"
        )
        ...
        ax.set_title(
            "Cluster Composition by Group (Percentages)", fontsize=14, fontweight="bold"
        )
        ax.set_xlabel(col1, fontsize=12)
