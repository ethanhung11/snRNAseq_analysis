...
def cell2cell_interactions(adata:AnnData,
                           ...
                           key="ccc",
                           resource_opts:Iterable[str]=['baccin2019', 'cellcall', 'cellchatdb', 'cellinker', 'cellphonedbv5', 'cellphonedb', 'celltalkdb', 'connectomedb2020', 'consensus', 'embrace', 'guide2pharma', 'hpmr', 'icellnet', 'italk', 'kirouac2010', 'lrdb', 'ramilowski2015', 'mouseconsensus'],
    ...
    rank_aggregate_custom(
        adata, groupby=cell_group, layer='normalized', use_raw=False, key_added=key, de_method='wilcoxon',
        return_all_lrs=True, verbose=True, n_jobs=cores, resource=ccc_db[["ligand", "receptor"]]
    )

    # save reference database info
    adata.uns[key] = adata.uns[key].merge(ccc_db[['ligand', 'receptor', 'db_sources']],
    ...
    if "scaled_weight" in adata.uns[key].columns: # Connectome
        ccc_filters.append(adata.uns[key]["scaled_weight"] > adata.uns[key]["scaled_weight"].quantile(.95))
    if "spec_weight" in adata.uns[key].columns: # NATMI
        ccc_filters.append(adata.uns[key]["spec_weight"] > adata.uns[key]["spec_weight"].quantile(.95))
    ...
    if "expr_prod" in adata.uns[key].columns: # NATMI/Connectome
        ccc_filters.append(adata.uns[key]["expr_prod"] > adata.uns[key]["expr_prod"].quantile(.95))
