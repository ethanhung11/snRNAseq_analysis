...
def Filter_QC(
    ...
    adata = adata[adata.obs['pct_counts_mt'] <= mt_ratio,:]
    adata = adata[adata.obs['pct_counts_ribo'] <= ribo_ratio,:]
...
def Filter_Doublet(
    ...
    # https://scanpy.readthedocs.io/en/stable/tutorials/basics/clustering.html#doublet-detection
        ...
        adata.obs['prediction'] = solo.predict(soft=False).prediction.map({'singlet':0, 'doublet':1}).astype(bool)
    else:
...
def FindVariableGenes(
    adata: AnnData | List[AnnData],
    kind: Literal["seurat", "seurat_v3", "seurat_v3_paper", "cell_ranger", "pearson", "deviant"] = "seurat",
    ...
    if kind == "seurat" or kind == "cell_ranger":
        sc.pp.highly_variable_genes(
            adata, batch_key=batch_column, flavor=kind, layer="normalized", n_top_genes=n_features,
        )

    elif kind == "seurat_v3" or kind == "seurat_v3_paper":
        sc.pp.highly_variable_genes(
            adata, batch_key=batch_column, flavor=kind, layer="counts", n_top_genes=n_features,
        )
    ...
    **kwargs,
    ...
    if kind == "harmony":
        ...
        sce.pp.harmony_integrate(
            adata, key=batch_column, max_iter_harmony=50, adjusted_basis="integrated", **kwargs
        )
        ...
        scvi.model.SCVI.setup_anndata(input_adata, layer="counts", batch_key=batch_column)
    ...
    #LocalMAP
    lm = LocalMAP()
    adata.obsm["LocalMAP"] = lm.fit_transform(adata.obsm['integrated'])
